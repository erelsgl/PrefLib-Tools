...
def findNecessarilyFairAllocation(prefProfile, isFair):
	"""
	INPUT:
	...
	if agentCount>3:
		raise ValueError("Currently findNecessarilyProportionalAllocation works only for at most three agents")
	itemCount = prefProfile.itemCount
	itemsPerAgent = itemCount // agentCount

	# First necessary condition for NPR allocation:  it is possible to give each agent the same num of items:
